...
export async function registerMonitoringRoutes(fastify) {
    /**
     * 获取错误统计信息
     */
    fastify.get('/api/monitoring/errors/stats', async (request, reply) => {
        try {
            ...
            await enhancedErrorHandler.handleError(error, 'GET /api/monitoring/errors/stats');
            reply.code(500);
            return {
                success: false,
    ...
    fastify.get('/api/monitoring/errors/recent', async (request, reply) => {
        try {
            ...
            await enhancedErrorHandler.handleError(error, 'GET /api/monitoring/errors/recent');
            reply.code(500);
            return {
                success: false,
    ...
    fastify.post('/api/monitoring/errors/clear', async (request, reply) => {
        try {
            ...
            await enhancedErrorHandler.handleError(error, 'POST /api/monitoring/errors/clear');
            reply.code(500);
            return {
                success: false,
    ...
    fastify.get('/api/monitoring/health', async (request, reply) => {
        try {
            ...
            await enhancedErrorHandler.handleError(error, 'GET /api/monitoring/health');
            reply.code(500);
            return {
                success: false,
    ...
    fastify.get('/api/monitoring/config', async (request, reply) => {
        try {
            ...
            await enhancedErrorHandler.handleError(error, 'GET /api/monitoring/config');
            reply.code(500);
            return {
                success: false,
    ...
    if (environment.server.nodeEnv === 'development') {
        fastify.post('/api/monitoring/test-error', async (request, reply) => {
            try {
                ...
                await enhancedErrorHandler.handleError(testError, `Test error - ${errorType}`);
                return {
                    success: true,
