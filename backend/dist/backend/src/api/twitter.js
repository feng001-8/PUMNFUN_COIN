...
// Twitter API路由
export async function twitterRoutes(fastify, twitterService) {
    // 获取Twitter情绪统计
    fastify.get('/api/twitter/sentiment/stats', async (request, reply) => {
        try {
            if (!twitterService) {
    ...
    // 获取特定代币的Twitter情绪数据
    fastify.get('/api/twitter/sentiment/:tokenAddress', async (request, reply) => {
        try {
            if (!twitterService) {
                return reply.code(503).send({
                    success: false,
    ...
    // 获取Twitter影响者列表
    fastify.get('/api/twitter/influencers', async (request, reply) => {
        try {
            if (!twitterService) {
                return reply.code(503).send({
                    success: false,
    ...
    // 添加新的影响者
    fastify.post('/api/twitter/influencers', async (request, reply) => {
        try {
            if (!twitterService) {
                return reply.code(503).send({
                    success: false,
    ...
    // 获取Twitter服务状态
    fastify.get('/api/twitter/status', async (request, reply) => {
        try {
            if (!twitterService) {
                return reply.send({
                    success: true,
    ...
    // 手动触发Twitter数据收集
    fastify.post('/api/twitter/collect', async (request, reply) => {
        try {
            if (!twitterService) {
                return reply.code(503).send({
                    success: false,
