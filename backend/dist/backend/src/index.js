...
if (environment.twitter?.bearerToken) {
    ...
    }, db, sentimentAnalyzer, tokenMapping);
}
// 监控界面路由
...
const start = async () => {
    try {
        ...
        `);
                stmt.run(token.address, token.name, token.symbol, token.decimals || 6, token.totalSupply || '1000000000', token.createdTimestamp || new Date().toISOString(), token.creator || '', token.initialLiquidity || 0, JSON.stringify({
        ...
        await enhancedErrorHandler.handleError(err, 'start');
        process.exit(1);
    }
};
// 优雅关闭
