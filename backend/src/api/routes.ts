...
import type { TokenInfo, PriceData, TradingData, Alert } from '../../../shared/types/index.js'
import { enhancedErrorHandler } from '../utils/enhanced-error-handler.js'

export async function registerRoutes(fastify: FastifyInstance, db: DatabaseManager, twitterService?: TwitterAPIService | null) {
  // 注册监控路由
  ...
  fastify.get('/api/data-source/status', async (request, reply) => {
    try {
      ...
      await enhancedErrorHandler.handleError(error as Error, 'GET /api/data-source/status')
      reply.code(500)
      return {
        success: false,
  ...
  fastify.get('/api/tokens', async (request, reply) => {
    try {
      ...
      const tokens = tokensStmt.all(...params, limit, offset) as any[]
      ...
      await enhancedErrorHandler.handleError(error as Error, 'GET /api/tokens')
      reply.code(500)
      return {
        success: false,
  ...
  fastify.get('/api/tokens/:address', async (request, reply) => {
    try {
      ...
      const token = stmt.get(address) as any
      ...
      const priceStmt = database.prepare(`
        ...
        LIMIT 1
      `)
      const latestPrice = priceStmt.get(address) as any
      ...
      const tradingStmt = database.prepare(`
        ...
        LIMIT 1
      `)
      const latestTrading = tradingStmt.get(address) as any
      ...
      await enhancedErrorHandler.handleError(error as Error, `GET /api/tokens/${request.params}`)
      reply.code(500)
      return {
        success: false,
  ...
  fastify.get('/api/tokens/:address/price', async (request, reply) => {
    try {
      ...
      const priceData = stmt.all(address, limit) as any[]
      ...
      await enhancedErrorHandler.handleError(error as Error, `GET /api/tokens/${request.params}/price`)
      reply.code(500)
      return {
        success: false,
  ...
  fastify.get('/api/tokens/:address/trading', async (request, reply) => {
    try {
      ...
      const tradingData = stmt.all(address, limit) as any[]
      ...
      await enhancedErrorHandler.handleError(error as Error, `GET /api/tokens/${request.params}/trading`)
      reply.code(500)
      return {
        success: false,
  ...
  fastify.get('/api/alerts', async (request, reply) => {
    try {
      ...
      const alerts = alertsStmt.all(...params, limit, offset) as any[]
      ...
      await enhancedErrorHandler.handleError(error as Error, 'GET /api/alerts')
      reply.code(500)
      return {
        success: false,
  ...
  fastify.patch('/api/alerts/:id/read', async (request, reply) => {
    try {
      ...
      await enhancedErrorHandler.handleError(error as Error, `PATCH /api/alerts/${request.params}/read`)
      reply.code(500)
      return {
        success: false,
  ...
  fastify.get('/api/kols', async (request, reply) => {
    try {
      ...
      await enhancedErrorHandler.handleError(error as Error, 'GET /api/kols')
      reply.code(500)
      return {
        success: false,
  ...
  fastify.get('/api/kols/:address/transactions', async (request, reply) => {
    try {
      ...
      const rows = stmt.all(address, limit) as any[]
      ...
      await enhancedErrorHandler.handleError(error as Error, `GET /api/kols/${request.params}/transactions`)
      reply.code(500)
      return {
        success: false,
   ...
   fastify.get('/api/sentiment/:tokenAddress', async (request, reply) => {
     try {
       ...
       await enhancedErrorHandler.handleError(error as Error, `GET /api/sentiment/${request.params}`)
       reply.code(500)
       return {
         success: false,
   ...
   fastify.get('/api/analysis/:tokenAddress', async (request, reply) => {
     try {
       ...
       const techRow = techStmt.get(tokenAddress) as any
       ...
       await enhancedErrorHandler.handleError(error as Error, `GET /api/analysis/${request.params}`)
       reply.code(500)
       return {
         success: false,
   ...
   fastify.get('/api/alert-configs', async (request, reply) => {
     try {
       ...
       await enhancedErrorHandler.handleError(error as Error, 'GET /api/alert-configs')
       reply.code(500)
       return {
         success: false,
   ...
   fastify.post('/api/alert-configs', async (request, reply) => {
     try {
       const { userId = 'default', name, description, conditions, actions, priority = 'medium' } = request.body as any
       ...
       await enhancedErrorHandler.handleError(error as Error, 'POST /api/alert-configs')
       reply.code(500)
       return {
         success: false,
   ...
   fastify.get('/api/stats', async (request, reply) => {
     try {
       ...
       const totalTokensResult = totalTokensStmt.get() as { count: number }
       ...
       `)
       const activeTokensResult = activeTokensStmt.get() as { count: number }
       ...
       `)
       const goldenDogsResult = goldenDogsStmt.get() as { count: number }
       ...
       const totalVolumeStmt = database.prepare(`
         ...
         WHERE timestamp >= datetime('now', '-24 hours')
       `)
       const totalVolumeResult = totalVolumeStmt.get() as { total: number }
       ...
       const avgPriceChangeStmt = database.prepare(`
         ...
         WHERE timestamp >= datetime('now', '-24 hours')
       `)
       const avgPriceChangeResult = avgPriceChangeStmt.get() as { avg: number }
       ...
       await enhancedErrorHandler.handleError(error as Error, 'GET /api/stats')
       reply.code(500)
       return {
         success: false,
