...
export class JupiterAPI {
  ...
  async getTokenPrice(tokenAddress: string): Promise<number | null> {
    try {
      ...
      const response = await httpClient.request<JupiterPriceResponse>(
        url + queryParams
      )
      ...
      errorHandler.handleError(error as Error, `获取代币价格: ${tokenAddress}`)
      return null
    }
  }
  ...
  async getMultipleTokenPrices(tokenAddresses: string[]): Promise<Map<string, number>> {
    ...
    try {
      ...
      errorHandler.handleError(error as Error, '批量获取价格')
    }
   ...
   * 计算价格变化百分比
   */
  async calculatePriceChanges(tokenAddress: string): Promise<PriceData | null> {
    try {
