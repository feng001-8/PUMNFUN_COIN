...
export class PumpPortalAPI {
  ...
  // 数据缓存
  private newTokens: TokenInfo[] = []
  private tokenTrades: Map<string, PumpPortalTradeEvent[]> = new Map()
  private maxCacheSize = 1000
  
  // 事件监听器
  private tokenListeners: ((token: TokenInfo) => void)[] = []
  private tradeListeners: ((trade: PumpPortalTradeEvent) => void)[] = []
  ...
  private handleMessage(message: any): void {
    try {
      ...
      if (message.mint && message.name && message.symbol) {
        const tokenInfo = this.transformTokenEvent(message as PumpPortalTokenEvent)
   ...
   */
  private transformTokenEvent(event: PumpPortalTokenEvent): TokenInfo {
    // 安全处理时间戳
    let createdAt: Date
    try {
   ...
   * 添加新代币到缓存
   */
  private addNewToken(token: TokenInfo): void {
   ...
   * 添加交易到缓存
   */
  private addTrade(trade: PumpPortalTradeEvent): void {
    // 添加到内存缓存
   ...
   * 保存交易数据到数据库
   */
  private saveTradeToDatabase(trade: PumpPortalTradeEvent): void {
    try {
   ...
   * 获取代币交易数据
   */
  async getTokenTrades(address: string, limit: number = 100): Promise<PumpPortalTradeEvent[]> {
  ...
  async calculateTradingData(address: string): Promise<TradingData | null> {
    try {
      ...
      errorHandler.handleError(error as Error, `计算交易数据: ${address}`)
      return null
    }
  }
  ...
  async checkGoldenDogCriteria(address: string): Promise<{
    ...
    try {
      ...
      errorHandler.handleError(error as Error, `检查金狗条件: ${address}`)
...
// 导出工厂函数来创建实例
export const createPumpPortalAPI = (db: DatabaseManager) => new PumpPortalAPI(db)

// 临时变量用于存储实例
let pumpPortalAPIInstance: PumpPortalAPI | null = null

// 导出获取实例的函数
export const getPumpPortalAPI = (): PumpPortalAPI => {
...
// 导出初始化函数
export const initializePumpPortalAPI = (db: DatabaseManager): PumpPortalAPI => {
  if (!pumpPortalAPIInstance) {
